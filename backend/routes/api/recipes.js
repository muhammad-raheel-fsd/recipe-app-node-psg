...
router.get("/", async (req, res) => {
  try {
    console.log("PASSWORD ========= ", await hashPassword("demouser123"));
    const recipes = await Recipe.findAll();
    if (recipes.length === 0) {
      return res
        .status(404)
        .json({ status: 404, message: "Recipes not found" });
    }
...
router.post("/addRecipe", async (req, res) => {
  const { name, restaurantId, notes, userId, image, ingredients, steps, tags } =
    req.body;
  try {
...
router.get("/reviews/:id", async (req, res) => {
  ...
  try {
    ...
    if (reviews.length === 0) {
      return res
        .status(404)
        .json({ status: 404, message: "Reviews not found" });
    }
